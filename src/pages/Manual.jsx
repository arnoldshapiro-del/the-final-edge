      ...
      <div className="flex gap-8 print:block">
        ...
        <main className="min-w-0 flex-1 space-y-12 print:space-y-8">
          ...
          <Section id="p1" pill="gold" label="PART ONE" sub="THE GAME PLAN" kicker="Read this every morning before you trade. This is the plan. Follow it exactly." primary>
            ...
            <P>Check the chart itself: the 2-minute must display session VWAP and the 9, 20 and 200 EMAs — all calculated on 2-minute data. Those four lines are the Gatekeeper, and you cannot pass a gate you can’t see. (Early note: in the first 20–30 minutes VWAP is still forming on very few data points, and the 2-min 200 mostly reflects yesterday — the open is a no-rush zone.) Mark your key levels. Set your max trades and your max loss for the day right now, while you’re calm — because the calm version of you is the only one you can trust to set them. Then drill the <X to="/trainer">Trainer</X> for five to ten minutes. Do not take a single trade until you’re calling the traps right — the counter-trend reversal attempt, the sideways range, the chase. That warm-up is you sharpening the exact reflex that cost you years.</P>
            ...
            <If><IfTag/>the trend is in place <Arrow/> <Str>check LOCATION — the Gatekeeper</Str> — before you hunt any flag. Long: the 2-min must be above VWAP, above the 9, above the 20 with the 20 rising, above the 200, with no VWAP/200 wall between entry and the first target. Short: the mirror. <strong className="text-coral">Any single miss = gate closed = no trade in that direction.</strong></If>
            ...
            <If><IfTag/>price pulls back on the 2-min and starts printing lower highs against the descending trendline <Arrow/> the bull flag is forming. <strong className="text-coral">The shape on its own is NEVER the entry.</strong> Flag quality: touching the 9 or 20 is healthy, but a 2-min CLOSE below the 20 — or below VWAP — kills the flag. You wait for the trendline close.</If>
          ...
          <Section id="p2" pill="emerald" label="PART TWO" sub="THE SYSTEM IN FULL" kicker="The locked plan, written out so you can find any piece in five seconds.">
            ...
            <Sub>The Gatekeeper — VWAP + EMA location filter (between trend and trigger)</Sub>
            <P>Live trading exposed the one hole in the old flow: the 15 and 5 can print higher highs and higher lows while the 2-minute trades <Em>below everything</Em>. Structure tells you where price <Str>has been</Str>; VWAP and the EMAs tell you <Str>who is in control right now</Str> — and both must agree. The full teaching lives in <X to="/learn/l-gate">Lesson 3</X>; this is the operating summary.</P>
            <UL>
              <LI><Str>The four lines (2-minute chart, 2-minute data):</Str> session VWAP — the institutional benchmark, resets daily; the 9 EMA — immediate momentum; the 20 EMA — the pullback line healthy flags ride; the 200 EMA — the regime line of the execution chart.</LI>
              ...
              <LI><Str>Long gate (ALL required):</Str> price above VWAP; above the 9; above the 20 with the 20 rising; above the 200. <Em>A+ when fully stacked:</Em> 9 above 20 above 200, VWAP below price. Any single miss = gate closed = no long.</LI>
              <LI><Str>Short gate (exact mirror):</Str> 15/5 LH/LL; price below VWAP, the 9, the 20 with the 20 falling, and the 200. A+ stacked 9 below 20 below 200.</LI>
              ...
              <LI><Str>Trap flags:</Str> a "bull flag" below the stack is the same shape with opposite context — from the other side it is often the bear flag. Flag quality: the pullback may TOUCH the 9 or 20, but 2-min candles must not CLOSE below the 20 during a bull flag, and never CLOSE below VWAP. Mirror for bear flags.</LI>
              ...
              <LI><Str>Early session:</Str> the 2-min 200 mostly reflects the prior session early on — which is exactly why VWAP (resets daily) is also mandatory; and in the first ~20–30 minutes VWAP itself is still forming. No exceptions to the all-four rule — only added patience.</LI>
              <LI><Str>Risk untouched:</Str> the Gatekeeper changes WHICH trades qualify — never how trades are managed. 6 contracts, the structure stop that defines your 1R, 2/2/2, and your daily limit (3 losses / 3 full stops = done): all exactly as this app already states them.</LI>
            </UL>
            ...
            <OL>
              ...
              <NLI><Str>The Gatekeeper is open:</Str> on the 2-min, price above VWAP and the 9/20/200 stack with the 20 rising, runway clear to T1. A flag forming below those lines is a trap flag — not this setup.</NLI>
              <NLI>The pullback holds above the prior swing low and ideally rides just above the rising 20 EMA — healthy. It may touch the 9 or 20, but no 2-min CLOSE below the 20 — and never below VWAP.</NLI>
            ...
            <UL>
              ...
              <LI>A flag on the wrong side of the stack — a bull flag below VWAP/9/20/200 or a bear flag above them. Same shape, opposite odds: the trap flag.</LI>
            ...
            <OL>
              ...
              <NLI><Str>LOCATION — the Gatekeeper is open</Str> (the second hard gate). Long: 2-min above VWAP, the 9, the 20 (rising) and the 200; short: the mirror; runway clear of VWAP/200 walls to T1. Conflict → the screen says <strong className="text-gold">GATE CLOSED — FLAT</strong>: no long AND no short.</NLI>
          ...
          <Section id="p3" pill="cyan" label="PART THREE" sub="THE PROGRAM, SCREEN BY SCREEN" kicker="Every part of the app, what it’s for, and how to actually use it.">
            ...
            <P><X to="/learn"><Str>Learn</Str></X> — the thirteen lessons. The curriculum, in the calm masterclass voice. Thirteen lessons, each with a “Lock-it-in” box. Several ship with live charts: <Em>the Gatekeeper — VWAP + EMA location filter</Em> (Lesson 3, with the green-light chart, the trap chart, the Reclaim Sequence and the decision flow), the flagship pullback → trendline-close entry chart (Lesson 4), the 2-2-2 ladder and the structure trail (Lesson 8), healthy vs. unhealthy pullback with the 20/9 EMAs (Lesson 5), and the two-step stop chart (Lesson 7). The <Em>Know The Odds</Em> card (Lesson 9) is the honest statistical reality of the system — why a loose flag is a coin flip, why a tight flag after a steep pole isn’t, and why the 15-min + location + 2-min-close filters are the whole edge. <Em>Method 3 — the Opening Range Break &amp; Retest</Em> (Lesson 10) applies the same flag logic to the most important level of the morning. <Em>How to use it:</Em> read all thirteen once in sequence in your first couple of days, then treat Learn as reference. Re-read any lesson the day after you break its rule.</P>
            ...
            <P><X to="/flashcards"><Str>Flashcards</Str></X> — twenty-six cards covering every mantra. Space-bar to flip, arrows to mark whether you knew it; the deck rotates your weakest cards first. <Em>How to use it:</Em> two minutes a day; keeps the vocabulary of the system on the tip of your tongue.</P>
          ...
          <Section id="p7" pill="muted" label="PART SEVEN" sub="PLAIN-LANGUAGE GLOSSARY" kicker="Every term in the system, in one line.">
            <dl className="space-y-3 mt-2">
              {[
                ...
                ['20 EMA / 9 EMA', 'exponential moving averages (smoothed average price). The 20 is your trend-health gauge; the 9 is what the runner trails. On the 2-minute they are also two of the Gatekeeper\'s four lines.'],
                ['The Gatekeeper', 'the location filter between trend and trigger. On the 2-minute: longs only with price above VWAP, the 9, the 20 (rising) and the 200; shorts the mirror. Structure says where price has been; the Gatekeeper says who is in control now. Both must agree.'],
                ...
                ['The stack / fully stacked', 'the 9, 20 and 200 EMAs in order. Fully stacked for longs: 9 above 20 above 200, with VWAP below price — the A+ location. Mirror for shorts.'],
                ['Trap flag', 'a flag shape on the wrong side of the stack — a "bull flag" below VWAP/9/20/200 is the same picture with opposite context; viewed from the other side it is often the bear flag. Same shape, opposite odds. A NO-trade, not a lower-odds trade.'],
                ...
                ['Opening range', 'the highest high and lowest low of the first 30 minutes of the session (9:30–10:00 AM ET). A map, not a trigger — its edges act as key support and resistance all day. Method 3 (Lesson 9) trades the break and retest of those edges.'],
